...
// Compute the weight
void
wei(arb_t result, arb_t x, arb_t a, arb_t b, arb_t c, slong prec)
{  
	arb_t ar, br;
	arb_init(ar);
	arb_init(br);
	...
	acb_set_arb_arb(bc, ar, br);
	acb_gamma(bc, bc, prec);
	
	//|A/B|^2
	acb_div(ac, ac, bc, prec);
	acb_abs(ar, ac, prec);
	arb_mul(result, ar, ar, prec);
...
void
hyp(acb_t result, arb_t x, arb_t a, arb_t b, arb_t c, arb_t n, slong prec)
{  
	arb_t ar;
	arb_init(ar);
...
// Compute the nfactor
void
nfactor(arb_t result, arb_t a, arb_t b, arb_t c, arb_t n, slong prec)
{  
	arb_t ar, br, cr, ab, ac, bc, pi;
	arb_init(ar);
	arb_init(br);
	arb_init(cr);
	arb_init(ab);
	...
	//gammma(n+a+b)*gamma(n+a+c)
	arb_add(ar, n, ab, prec);
	arb_gamma(ar, ar, prec);
	arb_add(br, n, ac, prec);
	arb_gamma(br, br, prec);
	arb_mul(ar, ar, br, prec);
	
	//2*Pi*gamma(a+b)^2*gamma(a+c)^2*gamma(n+1)*gamma(n+b+c)
	arb_mul_2exp_si(br, pi, 1);
	arb_gamma(cr, ab, prec);
	arb_sqr(cr, cr, prec);
...
void
integral_coeff(arb_t result, arb_t a, arb_t b, arb_t c, arb_t n, slong intlim, slong sw, slong prec)
{  
	arb_t ar, br, pi;
	arb_init(ar);
	arb_init(br);
...
int main(int argc, char *argv[])
{
    ...
    {
		arb_set_si(n, i);
		integral_coeff(res, a, b, c, n, intlim, sw, prec);
...
finish:
    ...
    if (result == EXIT_FAILURE)
    {
		...
		flint_printf("%s a b c numthreads sw \n\n", argv[0]);
		flint_printf(
    "This script computes the first 200 coefficients for the expansion of the Riemann Xi-function\n"
    "into the orthogonal Continuous Dual Hahn polynomials as described in the paper:\n"
    ...
    "-> 'a, b, c' are the required parameters.\n"
