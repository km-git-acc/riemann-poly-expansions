...
\begin{align}
\textbf{Legend:}& \notag \\
M_n(...) &= \text{the closed form of the orthogonality relation when $n=m$}. \notag \\
w_x(...) &= \text{the weight function}. \notag\\
f_n(...) &= \bigg(\text{terms before the hypergeom of the polynomial}\bigg)^2 / M_n(...)\notag \\
X_r^{\phi}(t,...) &= \text{the flow for polynomial family $\phi$. Optional : $(d)$ = $d$-th derivative.} \notag\\
\gamma_n(...) &= \text{coefficients of the expansion of the flow.} \notag \\
F(x) &= \text{the selected function to be expanded.} \notag  \\
-n(...)\,y(t) &= \text{the PDE or DDE for $y(t)=$ the polynomial under study.} \notag \\
\frac{\mathrm{d}}{\mathrm{d} r}z_k(r) &= \text{the first derivative of the $k$-th zero at time $r$ in the flow.} \notag \\
 \sum_{j \ne k}^{'} \frac{1}{z_k-z_j} &= \left(\sum_{j =1}^{k-1} \left(\frac{1}{z_k-z_j}\right)+\left(\frac{1}{z_k+z_j}\right)\right) +\left(\sum_{j = k+1}^{\infty} \left(\frac{1}{z_k-z_j}\right)+\left(\frac{1}{z_k+z_j}\right)\right)+\left(\frac{1}{2\,z_k} \right) \notag  \\
  \sum_{j \ne k}^{'} \frac{1}{2\pi k-2\pi j} &= -\frac{1}{2\pi k} \quad \text{for } F=\Xi_i \text{ at } r=0 \notag \\
\prod_{j \ne k}^{'} \frac{z_k+c-z_j}{z_k-z_j} &= c\,\left(\prod_{j =1}^{k-1} \left(1+\frac{c}{z_k-z_j}\right)\,\left(1+\frac{c}{z_k+z_j}\right)\right)\,\left(\prod_{j = k+1}^{\infty} \left(1+\frac{c}{z_k-z_j}\right)\,\left(1+\frac{c}{z_k+z_j}\right)\right)\,\left(1+\frac{c}{2\,z_k} \right) \notag \\
...
\begin{align}
  ...
  H_n(t) &= i^n\,\frac{2^n\,\Gamma\left(\frac{n+1}{2}\right)}{\sqrt{\pi}}\,{}_1F_1\left(-\frac{n}{2}, \frac12,t^2\right)\\
  M_n(a) &= \sqrt{\pi}\,2^n\,n!\\
  w_x &= \mathrm{e}^{-x^2}\\
  f_n &= \frac{2\,\Gamma\left(\frac{n}{2}+\frac12\right)}{\pi\,\Gamma(\frac{n}{2}+1)} \\
  -n\,y(x) &= \frac12\,y''(x)-x\,y'(x) \\
  X^\mathcal{H}_r(t) &= \sum_{n=0}^\infty \gamma_{2n}\,{}_1F_1\left(-n, \frac12,t^2\right)\,\exp(-r)^{2n} =\Xi_{\frac{\exp(-r)^2-1}{4}} (\exp(-r)\,t) \\ 
  \gamma_n &= f_n\,\int_{0}^{\infty} F(x)\,w_x\,{}_1F_1\left(-\frac{n}{2}, \frac12,x^2\right)\,\mathrm{d}x \\
  \sum_{n=0}^\infty \gamma_n &= F(0) \\
\frac{\mathrm{d}}{\mathrm{d} r} z_k(r)&=-\sum_{j \ne k}^{'} \frac{1}{z_k(r)-z_j(r)} + z_k(r)\\
\frac{\mathrm{d}}{\mathrm{d} r} z_k(0)&= \frac{1}{2\,k\,\pi} + 2\,k\,\pi \quad \text{when } F=\Xi_i
\end{align}
\end{minipage}}
\begin{figure}[H]
  \noindent
...
\begin{align}
  ...
  L^{a}_n(x) &= \frac{(\alpha+1)_n}{n!}\, {}_1F_1(-n, a+1,x) \\
  M_n(a) &= \frac{\Gamma(\alpha+1+n)}{n!} \\
  w(x,a) &= \mathrm{e}^{-x}\,x^a\\
  f_n(a) &= \frac{\Gamma\left(n+a+1\right)}{\Gamma(a+1)^2\,n!} \\
  -n\,y(x) &= x\,y''(x)+(\alpha+1-x)\,y'(x) \\
  X^{\mathcal{L}\,(d)}_r(t,a) &= \frac{a}{(a)_{d+1}}\,\sum_{n=0}^\infty \gamma_n(a)\,(-n)_d\,{}_1F_1\left(d-n, a+d+1,t\right)\,\exp(-r)^n \\ 
  \gamma_n(a) &= f_n(a)\int_{0^+}^{\infty} F(x)\,w(x,a)\,{}_1F_1\left(-n, a+1,x\right)\,\mathrm{d}x \\
  \sum_{n=0}^\infty \gamma_n(a) &= \Xi(0) \\
  \frac{\mathrm{d}}{\mathrm{d} r} z_k(r)&=-2\,z_k(r)\,\sum_{j \ne k}^{'} \frac{1}{z_k(r)-z_j(r)} -(\alpha+1-z_k(r))\\
  \frac{\mathrm{d}}{\mathrm{d} r} z_k(0)&=2-(\alpha+1-2\,k\,\pi) \quad \text{when } F=\Xi_i
\end{align}
\end{minipage}}
\begin{figure}[H]
...
\begin{align}
  ...
  y_n(x,a) &= {}_2F_0\left(-n, n+a+1,[],-\frac{x}{2}\right) \\
  M_n(a) &= -\frac{2^{a+1}\,\Gamma(-n-a)\,n!}{2n+a+1}\\
  w_x(a) &= \mathrm{e}^{-\frac{2}{x}}\,x^a \\
  f_n(a) &= -\frac{\left(2n+a+1\right)}{2^{a+1}\,\Gamma(-n-a)\,n!} \\
  -n\,(n+a+1)\,y(x) &= -x^2\,y''(x)-\big((a+2)\,x+2\big)\,y'(x) \\ \label{besPDE}
  X^{\mathcal{B}\,(d)}_r(t,a) &= \frac{(-1)^d}{2^d}\sum_{n=0}^\infty \gamma_n(a)\frac{(n+a)_{d+1}(-n)_d}{n+a}{}_2F_0\left([d-n, n+a+d+1],[],-\frac{t}{2}\right)\exp\left(-r(n+a+1)\right)^n \\ \label{besflow}
  \gamma_n(a) &= f_n(a)\,\int_{0^+}^{\infty} F(x)\,w_x(a)\,{}_2F_0\left([-n, n+a+1],[],\frac{-x}{2}\right)\,\mathrm{d}x \\
  \sum_{n=0}^\infty \gamma_n(a) &= \Xi(0) \\
  \frac{\mathrm{d}}{\mathrm{d} r} z_k(r)&=2\,z_k(r)^2\,\sum_{j \ne k}^{'} \frac{1}{z_k(r)-z_j(r)} + \big((a+2)\,z_k(r)+2\big)\\
  \frac{\mathrm{d}}{\mathrm{d} r} z_k(0)&=-2\,(2\,k\,\pi)+ \big((a+2)\,(2\,k\,\pi)+2\big) \quad \text{when } F=\Xi_i
\end{align}
\end{minipage}}
\begin{figure}[H]
...
\begin{align}
  ...
  P_n^{(\lambda)}\left(x, \frac{\pi}{2}\right) &= i^n\,\frac{(2\lambda)_n}{n!}\,{}2F_1\left([-n, \lambda+i\,x],[2\lambda],2\right)\\
  M_n(\lambda) &= \frac{2\,\pi\,\Gamma(n+2\lambda)}{2^{2\lambda}\,n!} \\
  w_x(\lambda) &= \Gamma(\lambda+ix)\,\Gamma(\lambda-ix) \\
  f_n(\lambda) &= \frac{4^\lambda\,\Gamma(n+2\lambda)}{2\,\pi\,\Gamma(2\lambda)^2\,\Gamma(n+1)} \\
  -n\,y(x) &= -\frac12\,(\lambda-ix)\,y(x+i)+\lambda\,y(x)-\frac12\,(\lambda+ix)\,y(x-i) \\
  X^\mathcal{MP}_r(t,\lambda) &= \sum_{n=0}^\infty \gamma_{2n}(\lambda)\,{}_2F_1\left([-2n, \lambda+it],[2\lambda],2\right)\,\exp\left(-r\right)^{2n} \\ 
  \gamma_n(\lambda) &= f_n(\lambda)\,\int_{-\infty}^{\infty} F(x)\,w_x(\lambda)\,{}_2F_1\left([-n, \lambda+ix],[2\lambda],2\right)\,\mathrm{d}x \\
  \sum_{n=0}^\infty \gamma_n(\lambda) &= \Xi(i\lambda) \\
  \frac{\mathrm{d}}{\mathrm{d} r} z_k(r)&=  \left(\frac{\lambda-i\,z_k(r)}{2}\right)\,\prod_{j \ne k}^{'} \left(1+\frac{i}{z_k(r)-z_j(r)}\right)+\left(\frac{\lambda+i\,z_k(r)}{2}\right)\,\prod_{j \ne k}^{'} \left(1-\frac{i}{z_k(r)-z_j(r)}\right) \\
  \frac{\mathrm{d}}{\mathrm{d} r} z_k(0)&= \left(\frac{\lambda}{2}-k\,\pi\,i\right)\,ZP(k,i)+\left(\frac{\lambda}{2}+k\,\pi\,i\right)\,ZP(k,-i)\quad \text{when } F=\Xi_i 
\end{align}
\end{minipage}}
\begin{figure}[H]
...
\begin{align}
  ...
  M_n(\alpha,\beta) &= \frac{2^{\alpha+\beta+1}}{2n+\alpha+\beta+1}\,\frac{\Gamma(n+\alpha+1)\,\Gamma(n+\beta+1)}{\Gamma(n+\alpha+\beta+1)\,n!}\\
  w_x(\alpha,\beta) &= (1-x)^\alpha\,(1+x)^\beta \\
  f_n(\alpha,\beta) &= \frac{\Gamma(n+\alpha+1)\,\Gamma(n+\alpha+\beta+1)\,(2n+\alpha+\beta+1)}{2^{\alpha+\beta+1}\,\Gamma(\alpha+1)^2\,\Gamma(n+1)\,\Gamma(n+\beta+1)} \\
 -n\,(n+\alpha+\beta+1)\,y(x) &= (1-x^2)\,y''(x)+\big(\beta-\alpha-(\alpha+\beta+2)\,x\big)\,y'(x) \\
  X^{\mathcal{J}\,(d)}_r(t,\alpha,\beta) &= \frac{(-1)^d\,\alpha}{2^d\,(\alpha)_{d+1}}\,\sum_{n=0}^\infty \gamma_n(\alpha,\beta)\,\frac{(-n)_d\,(n+\alpha+\beta)_{d+1}}{n+\alpha+\beta}\\ &\times {}_2F_1\left([d-n, n+\alpha+\beta+d+1],[\alpha+d+1],\frac{1-t}{2}\right)\,\exp\left(-r(n+\alpha+\beta+1)\right)^n \\ 
  \gamma_n(\alpha,\beta) &= f_n(\alpha,\beta)\,\int_{-1^+}^{1^-} \Xi(x)\,w_x(\alpha,\beta)\,{}_2F_1\left([-n, n+\alpha+\beta+1],[\alpha+1],\frac{1-x}{2}\right)\,\mathrm{d}x \\
  \sum_{n=0}^\infty \gamma_n(\alpha,\beta) &= \Xi(1) \\
  \frac{\mathrm{d}}{\mathrm{d} r} z_k(r)&=-2\,(1-z_k(r)^2)\,\sum_{j \ne k}^{'} \frac{1}{z_k(r)-z_j(r)} - \big(\beta-\alpha-(\alpha+\beta+2)\,z_k(r)\big)\\
  \frac{\mathrm{d}}{\mathrm{d} r} z_k(0)&=2\,\frac{(1-(2\,k\,\pi)^2)}{2\,k\,\pi}-\big(\beta-\alpha-(\alpha+\beta+2)\,(2\,k\,\pi)\big) \quad \text{when } F=\Xi_i
\end{align}
\end{minipage}}
\begin{figure}[H]
...
\begin{align}
  ...
  P_n(x,v,N) &= \frac{(-2i)^n\,(-N+iv)_n}{(n-2N-1)_n}\,{}_2F_1\left([-n, n-2N-1],[-N+iv],\frac{1-ix}{2}\right)\\
  M_n(v,N) &= \frac{2\pi\,\Gamma(2N+1-2n)\,\Gamma(2N+2-2n)\,2^{2n-2N-1}\,n!}{\Gamma(2N+2-n)\,|\Gamma(N+1-n+iv)|^2} \\
  w_x(v,N) &= \left(1+x^2\right)^{-N-1}\,\mathrm{e}^{2a\,\arctan(x)}\\
  f_n(v,N) &=(-1)^n \frac{4^N\,(2N+1-2n)\,\Gamma(n-N+iv)^2\,|\Gamma(N+1-n+iv)|^2}{\pi\,n!\,\Gamma(2N+2-n)\,\Gamma(-N+iv)^2} \\
  -n\,(n-2N-1)\,y(x) &= -(1+x^2)\,y''(x)-2\,\big(v-Nx\big)\,y'(x) \\
  X^{\mathcal{PJ}\,(d)}_r(t,v,N) &= \frac{(-i)^d}{2^d\,(-N+iv)_d}\,\sum_{n=0}^\infty \gamma_n(v,N)\,(-n)_d\,(n-2N-1)_d\\ &\times{}_2F_1\left([d-n, n-2N-1+d],[d-N+iv],\frac{1-it}{2}\right)\,\exp\left(-r(n-2N-1)\right)^n \\ 
...
\begin{align}
  ...
  M_n(a,b,c) &= 2\pi\,n!\,\Gamma(n+a+b)\,\Gamma(n+a+c)\,\Gamma(n+b+c) \\
  w_x(a,b,c) &= \left|\frac{\Gamma(a+ix)\,\Gamma(b+ix)\,\Gamma(c+ix)}{\Gamma(2ix)}\right|^2 \\
  f_n(a,b,c) &= \frac{\Gamma(n+a+b)\,\Gamma(n+a+c)}{2\,\pi\,\Gamma(a+b)^2\,\Gamma(a+c)^2\,n!\,\Gamma(n+b+c)} \\
...
\begin{align}
  ...
  \kappa_n &= n + a + b + c + d - 1 \\
  W_n(y^2;a,b,c,d) &= (a+b)_n(a+c)_n(a+d)_n {}_4F_3([-n, \kappa_n,a+iy, a- iy], [a+b, a+c, a+d], 1) \\
  M_n(a,b,c,d) &= \frac{2\pi(\kappa_n)_nn!\Gamma(n+a+b)\Gamma(n+a+c)\Gamma(n+a+d)\Gamma(n+b+c)\Gamma(n+b+d)\Gamma(n+c+d)}{\Gamma(2n+a+b+c+d)}\\
  w_x(a,b,c,d) &= \left|\frac{\Gamma(a+ix)\,\Gamma(b+ix)\,\Gamma(c+ix)\,\,\Gamma(d+ix)}{\Gamma(2ix)}\right|^2 \\
  f_n(a,b,c,d) &= \frac{(a+b)_n^2\,(a+c)_n^2\,(a+d)_n^2\,\Gamma(2n+a+b+c+d)}{2\pi\,(\kappa_n)_n\Gamma(n+a+b)\Gamma(n+a+c)\Gamma(n+a+d)n!\Gamma(n+b+c)\Gamma(n+b+d)\Gamma(n+c+d)}\\
  -n\,\kappa_n\,y(x) &= -B(x)\,y(x+i)+\left(B(x)+D(x)\right)\,y(x)-D(x)\,y(x-i) \\ \label{wildde}
  B(x)&=\frac{(a-ix)\,(b-ix)\,(c-ix)\,(d-ix)}{2ix\,(2ix-1)} \\
  D(x)&=\frac{(a+ix)\,(b+ix)\,(c+ix)\,(d+ix)}{(2ix\,(2ix+1)} \\
  X^\mathcal{W}_r(t,a,b,c,d) &= \sum_{n=0}^\infty \gamma_n(a,b,c,d)\,{}_4F_3\left(\left[-n, \kappa_n,a+it, a- it\right], \left[a+b, a+c, a+d\right], 1\right)\,\exp\left(-r\,\kappa_n\right)^n \\ \label{wilflow}
  \gamma(n,a,b,c,d) &= f_n(a,b,c,d)\int_{0}^{\infty} F(x)\,w_x(){}_4F_3\left(\left[-n, \kappa_n,a+ix, a- ix\right], \left[a+b, a+c, a+d\right], 1\right)\,\mathrm{d}x \\
  \sum_{n=0}^\infty \gamma_n(a,b,c,d) &= \Xi(\pm ia) \\
  \frac{\mathrm{d}}{\mathrm{d} r} z_k(r)&=B\big(Z_k(r)\big)\,\prod_{j \ne k}^{'} \left(1+\frac{i}{z_k(r)-z_j(r)}\right)+D\big(Z_k(r)\big)\,\prod_{j \ne k}^{'} \left(1-\frac{i}{z_k(r)-z_j(r)}\right) \\
  \frac{\mathrm{d}}{\mathrm{d} r} z_k(0)&= B\big(2\,k\,\pi\big)\,ZP(k,i)+D\big(2\,k\,\pi)\,ZP(k,-i)\quad \text{when } F=\Xi_i 
\end{align}
\end{minipage}}
