...
We observed that also the Jacobi polynomials possess the property that the coefficients at odd indices are zero (but only when $\alpha\verifiedeq \beta$). Using equation 9.8.13 in \cite{koe} yields its toy expression for $\alpha \verifiedeq \beta\verifiedeq \frac12$:
\begin{align}
...
X_r^{\phi}(t;...)\left[F(t)\right] &\verifiedeq \text{the flow for polynomial family $\phi$. Optional : $(d)$ = $d$-th derivative.} \notag\\
...
\begin{align}
  ...
  X^\mathcal{H}_r(t)\left[F(t)\right] &\verifiedeq \sum_{n=0}^\infty \gamma_{2n}\,{}_1F_1\left(-n, \frac12,t^2\right)\,\exp(-r)^{2n} =\Xi_{\frac{\exp(-r)^2-1}{4}} (\exp(-r)\,t) \\ 
...
\begin{align}
  ...
  X^{\mathcal{L}\,(d)}_r(t;a)\left[F(t)\right] &\verifiedeq \frac{a}{(a)_{d+1}}\,\sum_{n=0}^\infty \gamma_n(a)\,(-n)_d\,{}_1F_1\left(d-n, a+d+1,t\right)\,\exp(-r)^n \\ 
...
\begin{align}
  ...
  X^{\mathcal{B}\,(d)}_r(t;a)\left[F(t)\right] &\verifiedeq \frac{(-1)^d}{2^d}\sum_{n=0}^\infty \gamma_n(a)\frac{(n+a)_{d+1}(-n)_d}{n+a}{}_2F_0\left([d-n, n+a+d+1],[],-\frac{t}{2}\right)\exp\left(-r(n+a+1)\right)^n \\ \label{besflow}
...
\begin{align}
  ...
  P_n^{(\lambda)}\left(x; \frac{\pi}{2}\right) &\defeq i^n\,\frac{(2\lambda)_n}{n!}\,{}_2F_1\left([-n, \lambda+i\,x],[2\lambda],2\right)\\
  ...
  X^\mathcal{MP}_r(t;\lambda)\left[F(t)\right] &\verifiedeq \sum_{n=0}^\infty \gamma_{2n}(\lambda)\,{}_2F_1\left([-2n, \lambda+it],[2\lambda],2\right)\,\exp\left(-r\right)^{2n} \\ 
...
\begin{align}
  ...
  P_n(x;\alpha,\beta) &\defeq \frac{(\alpha+1)_n}{n!}\,{}_2F_1\left([-n, n+\alpha+\beta+1],[\alpha+1],\frac{1-x}{2}\right) \\
 ...
 -n\,(n+\alpha+\beta+1)\,y(x) &\verifiedeq (1-x^2)\,y''(x)+\big(\beta-\alpha-(\alpha+\beta+2)\,x\big)\,y'(x) \\
  X^{\mathcal{J}\,(d)}_r(t;\alpha,\beta)\left[F(t)\right] &\verifiedeq \frac{(-1)^d\,\alpha}{2^d\,(\alpha)_{d+1}}\,\sum_{n=0}^\infty \gamma_n(\alpha,\beta)\,\frac{(-n)_d\,(n+\alpha+\beta)_{d+1}}{n+\alpha+\beta}\\ &\times {}_2F_1\left([d-n, n+\alpha+\beta+d+1],[\alpha+d+1],\frac{1-t}{2}\right)\,\exp\left(-r(n+\alpha+\beta+1)\right)^n \\ 
...
\begin{align}
  ...
  X^{\mathcal{PJ}\,(d)}_r(t;v,N)\left[F(t)\right] &\verifiedeq \frac{(-i)^d}{2^d\,(-N+iv)_d}\,\sum_{n=0}^\infty \gamma_n(v,N)\,(-n)_d\,(n-2N-1)_d\\ &\times{}_2F_1\left([d-n, n-2N-1+d],[d-N+iv],\frac{1-it}{2}\right)\,\exp\left(-r(n-2N-1)\right)^n \\ 
...
\begin{align}
  ...
  X^\mathcal{CDH}_r(t;a,b,c)\left[F(t)\right] &\verifiedeq \sum_{n=0}^\infty \gamma_n(a,b,c)\,{}_3F_2\left([-n, a+it, a-it],[a+b,a+c],1\right)\,\exp\left(-r\right)^n \\ 
...
\begin{align}
 ...
 -n\,\kappa_n\,y(x) &\verifiedeq -B(x)\,y(x+i)+\left(B(x)+D(x)\right)\,y(x)-D(x)\,y(x-i) \\
  ...
  X^\mathcal{CH}_r(t;a,b,c,d)\left[F(t)\right] &\verifiedeq \sum_{n=0}^\infty \gamma_n(a,b,c,d)\,{}_3F_2\left([-n, \kappa_n, a+it],[a+c,a+d],1\right)\,\exp\left(-r\,\kappa_n\right)^n \\ 
...
\begin{align}
  ...
  X^\mathcal{W}_r(t;a,b,c,d)\left[F(t)\right] &\verifiedeq \sum_{n=0}^\infty \gamma_n(a,b,c,d)\,{}_4F_3\left(\left[-n, \kappa_n,a+it, a- it\right], \left[a+b, a+c, a+d\right], 1\right)\,\exp\left(-r\,\kappa_n\right)^n \\ \label{wilflow}
...
\begin{table}[H]
  \begin{center}
    ...
    \begin{tabular}{l|l|c|} 
      ...
      \hline
      $H_{2n}(t)$ & $\displaystyle \int_{-\infty}^{\infty} \Phi(x)\,x^n\,\mathrm{e}^{-x^2/4}\,\mathrm{d}x $  &$\Xi(t)$ \\  
      $L^{-1/2}_n(t^2)$ & $\displaystyle  \int_{-\infty}^{\infty} \Phi(x)\,x^{2n}\,\mathrm{e}^{-x^2/4}\,\mathrm{d}x $  &$\Xi(t)$ \\
      $L^{\alpha}_n(t)$ & $\displaystyle  \int_{-\infty}^{\infty} \Phi(x)\,\left(\frac{ix}{ix+1}\right)^n\,\left(\frac{1}{ix+1}\right)^{\alpha+1}\,\mathrm{d}x $  &$\Xi(t)$ \\
      $P_{2n}^{(a)}\left(t;\frac{\pi}{2}\right)$ & $\displaystyle \int_{-\infty}^{\infty} \Phi(x)\,\tanh\,\left(\frac{x}{2}\right)^n\,\,\,\frac{\textrm{e}^{a\,x}}{(\textrm{e}^x+1)^{2a}}\,\mathrm{d}x $  &$\Xi(t)$ \\ 
      $p_{2n}\left(\frac{t}{2};\frac{a}{2},\frac{a+1}{2},\frac{a}{2},\frac{a+1}{2}\right)$ & $\displaystyle \int_{-\infty}^{\infty} \Phi(x)\,\tanh\,\left(\frac{x}{2}\right)^n\,\,\,\frac{\textrm{e}^{a\,x}}{(\textrm{e}^{x}+1)^{2a}}\,\mathrm{d}x $  &$\Xi(t)$ \\   
      $p_{2n}\left(t;a,b,a,b\right)$ & $\displaystyle \int_{-\infty}^{\infty} \Phi(x)\,\tanh\,\left(\frac{x}{2}\right)^n\,\,\,\frac{\textrm{e}^{a\,x}}{(\textrm{e}^{x}+1)^{2a}}\,hg1(x,n,a,b)\,\mathrm{d}x$  &$\Xi(t)$ \\                  
      $S_{n}\left(t^2;a,\frac12,0\right)$ & $\displaystyle \int_{-\infty}^{\infty} \Phi(x)\,\tanh\,\left(\frac{x}{2}\right)^{2n}\,\frac{\textrm{e}^{a\,x}}{(\textrm{e}^x+1)^{2a}}\,\mathrm{d}x$  &$\Xi(t)$ \\
      $S_{n}\left(t^2;a,b,\frac12\right)$ & $\displaystyle \int_{-\infty}^{\infty} \Phi(x)\,\tanh\,\left(\frac{x}{2}\right)^{2n}\,\frac{\textrm{e}^{a\,x}}{(\textrm{e}^x+1)^{2a}}\,hg2(x,n,a,b) \,\mathrm{d}x $  &$\Xi(t)$ \\
      $W_n\left(t^2;a,b,\frac12,0\right)$ & $\displaystyle \int_{-\infty}^{\infty} \Phi(x)\,\tanh\left(\frac{x}{2}\right)^{2n}\,\frac{\mathrm{e}^{ax}}{(\mathrm{e}^x+1)^{2a}}\,hg3(x,n,a,b)\,\mathrm{d}x$ &$\Xi(t)$ \\
